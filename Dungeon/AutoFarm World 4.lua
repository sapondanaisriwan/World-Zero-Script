...
elseif inDungeon then
	...
	function specialMob()
		...
		elseif game.PlaceId == 4646475570 then
			for i,v in pairs(game.workspace:GetChildren()) do
				if v.Name == 'Gate' and v:FindFirstChild('HealthProperties').Health.Value > 0 then
					table.insert(mob, v)
					table.insert(pos, v.PrimaryPart.Position)
					special = v
				end
	...
	function nearest()
		...
		elseif game.PlaceId == 4646473427 then
			warn('4')
			for _,o in pairs(ms.TowerLegs:GetChildren()) do
				if o:FindFirstChild('HealthProperties') and o.HealthProperties.Health.Value > 0 then
					target, _, __ = specialMob()
					return target
				end
	...
	function main2()
		while wait() do
			...
			if target ~= nil then
				...
				if target.Name == 'BOSSHogRider' then
					move(target, math.random(0,20), 15, math.random(0,20))
