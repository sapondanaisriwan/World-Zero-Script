...
local Classes = {
	...
	['Archer']			= {'Archer','PiercingArrow1','PiercingArrow2','PiercingArrow3','SpiritBomb','MortarStrike1','MortarStrike2','MortarStrike3','MortarStrike4','MortarStrike5','MortarStrike6','MortarStrike7'}
}
...
elseif inDungeon then
	...
	function nearest()
		...
		for _, v in pairs(game.workspace.Mobs:GetChildren()) do
			if v:FindFirstChild('Collider') and v:FindFirstChild("HealthProperties").Health.Value > 0 then
				...
				if v.HealthProperties.Health.Value == 0 then
					v:Destroy()
				end
			end
	...
	function getMobInArea()
		...
		for i,v in pairs(game:GetService("Workspace").Mobs:GetChildren()) do
		   if v:FindFirstChild('HealthProperties') and v.HealthProperties.Health.Value > 0 and v.Parent.Parent.Name ~= 'Characters' then --v:FindFirstChild('Collider') and 
				local distance = (v.PrimaryPart.Position - plr.Position).magnitude
	...
	function main2()
		while wait() do
			...
			if target ~= nil then
				...
				if target.Name == 'BOSSHogRider' or target.Name == 'BlossomTree' then
