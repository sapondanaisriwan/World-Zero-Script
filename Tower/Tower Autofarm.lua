...
elseif inTower then
    ...
    wait(3)
    -- auto restart
    player.PlayerGui.MainGui.TowerFinish.Close.Changed:Connect(function(v)
    ...
    function monster() -- get monster in distance
        ...
        for i,v in pairs(game:GetService("Workspace").Mobs:GetChildren()) do
           if v:FindFirstChild('Collider') and v:FindFirstChild('HealthProperties') and v.HealthProperties.Health.Value > 0 then --v.HealthProperties.Health > 0 then
                ...
                -- if v.Name == 'BOSSZeus' then
                --     Mission.Teleporter.CanCollide = true
                --     Mission.Teleporter.CFrame = plr.CFrame
                --     wait(.2)
                --     Mission.Teleporter.CanCollide = false
                -- end
           end
        end
        return mob, pos
    ...
    function spawnMob()
        while true do
            ...
            if waveText:find('WAVE') then
                col.CFrame = Mission.WaveStarter.CFrame
                torso.CFrame = Mission.WaveStarter.CFrame
            end
            wait()
            if workspace:FindFirstChild('Map') then
                for _,a in ipairs(game:GetService("Workspace").Map:GetChildren()) do
                            ...
                            if b:FindFirstChild('Spawns') then
                                for _,v in ipairs(b.Spawns:GetChildren()) do
                                    if (v:IsA('Part') and v.Name == 'Spawn') then
                                        col.CFrame = v.CFrame * CFrame.new(0 , 20, 0)
                                        torso.CFrame = v.CFrame * CFrame.new(0, 20, 0)
                                        wait(0.9)
                                    end
                                end
                            end
